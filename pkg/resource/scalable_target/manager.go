...
// +kubebuilder:rbac:groups=applicationautoscaling.services.k8s.aws,resources=scalabletargets/status,verbs=get;update;patch

var lateInitializeFieldNames = []string{"RoleARN"}
...
type resourceManager struct {
	...
	sdkapi svcsdkapi.ApplicationAutoScalingAPI
}
...
func (rm *resourceManager) ARNFromName(name string) string {
	return fmt.Sprintf(
		"arn:aws:applicationautoscaling:%s:%s:%s",
		rm.awsRegion,
		rm.awsAccountID,
		name,
	)
...
func (rm *resourceManager) lateInitializeFromReadOneOutput(
	...
	if observedKo.Spec.RoleARN != nil && latestKo.Spec.RoleARN == nil {
		latestKo.Spec.RoleARN = observedKo.Spec.RoleARN
	}
